...
`);
      ...
      const width = 1860; //container.scrollWidth;
      const height = 900; // container.scrollHeight || 500;
      ...
      const initGraph = (data) => {
        ...
        //   this.collapseSibNode(item, false);
        // });
      };
    ...
    collapseSibNode(item, collapsed) {
      ...
      if (farNode) {
        ...
        if (siblingNode.length > 0 && siblingNode) {
          this.collapseNode(siblingNode);
        }
      } else {
        // 当前是根节点
        this.collapseNode(item._cfg.children);
      }
    ...
    collapseNode(children) {
      children.forEach((el) => {
        ...
        if (isChildren) {
          this.collapseNode(el._cfg.children);
          // 先关闭子节点后关闭父节点
          data.collapsed = true;
        }
      });
